...
export async function stooqSeries(code, days = 200) {
  // stooq.com and stooq.pl serve the same data; if one refuses the runner's
  // IP the other sometimes does not.
  for (const host of ['stooq.com', 'stooq.pl']) {
    const url = `https://${host}/q/d/l/?s=${encodeURIComponent(code.toLowerCase())}.au&i=d`;
    const res = await safeFetch(url, {}, 15000, 0);
    if (!res) continue;

    const text = await res.text();
    if (!text) { noteError(url, 'empty response'); continue; }

    // Stooq answers 200 with a plain-text message when it throttles, so a
    // successful HTTP status is not enough to trust the body.
    if (/exceeded the daily hits limit/i.test(text)) {
      noteError(url, 'rate limited: "Exceeded the daily hits limit"');
      continue;
    }
    if (!/^Date,/i.test(text.trim())) {
      noteError(url, `unexpected body: ${text.trim().slice(0, 60)}`);
      continue;
    }
    ...
    if (out.length < 5) { noteError(url, `only ${out.length} rows parsed`); continue; }
    return out.slice(-days);
  }
  return null;
}

/** Why the last price-history attempt failed, for the Data health panel. */
export function historyDiagnostics() {
  const hosts = ['query1.finance.yahoo.com', 'stooq.com', 'stooq.pl'];
  const out = {};
  for (const h of hosts) if (lastError[h]) out[h] = lastError[h];
  return out;
}
